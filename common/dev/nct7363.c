...
uint8_t nct7363_init(sensor_cfg *cfg)
{
	...
	for (uint8_t i = 0; i < 4; i++) {
		offset = GPIO_00_to_03_Pin_Configuration_REG + i;
		val = (nct7363_init_arg_data->pin_type[3 + 4 * i] << 6) |
		      (nct7363_init_arg_data->pin_type[2 + 4 * i] << 4) |
		      (nct7363_init_arg_data->pin_type[1 + 4 * i] << 2) |
		      (nct7363_init_arg_data->pin_type[4 * i]);
	...
	for (uint8_t i = 0; i < 2; i++) {
		...
		offset = i ? GPIO1x_Input_Output_Configuration_REG :
			     GPIO0x_Input_Output_Configuration_REG;
		val = 0xFF;
		for (uint8_t j = 0; j < 8; j++) {
			if (nct7363_init_arg_data->pin_type[i * 8 + j] == 1) {
				val &= ~(1 << j);
			}
		}
		if (nct7363_write(cfg, offset, val))
		...
		/* set PWM output */
		val = 0;
		offset = i ? NCT7363_REG_PWM_CTRL_OUTPUT_8_to_15 :
			     NCT7363_REG_PWM_CTRL_OUTPUT_0_to_7;
		for (uint8_t j = 0; j < 8; j++) {
			if (nct7363_init_arg_data->pin_type[i * 8 + j] == NCT7363_PIN_TPYE_PWM) {
				val |= (1 << j);
			}
		...
		/*  set FANIN control */
		val = 0;
		offset = i ? NCT7363_REG_FANIN_CTRL2 : NCT7363_REG_FANIN_CTRL1;
		for (uint8_t j = 0; j < 8; j++) {
			if (nct7363_init_arg_data->pin_type[i * 8 + j] == NCT7363_PIN_TPYE_FANIN) {
				val |= (1 << j);
			}
